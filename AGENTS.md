...
- 译名/术语：优先官方中文（见 `GLOSSARY.md`）；对常见著名作曲家/指挥家，若 `GLOSSARY.md` 已收录对照则正文可省略括注原文；其他情况建议首次出现处保留原文以便核对
- 完成后：运行 `python3 -m scripts.manage.update_booklets_links_only` 刷新 `BOOKLETS.md` 的快捷链接与完成状态
...
- 对常见著名作曲家/指挥家：若已在 `GLOSSARY.md` 收录对照，则正文可直接用中文译名而不再括注原文；如不在且希望省略括注，先补充 `GLOSSARY.md` 再统一处理。
...
```bash
python3 -m scripts.translate.extract_booklet_english --booklet-dir 'booklets/<booklet-标题>'
```
...
```bash
python3 -m scripts.translate.audit_booklet_en_language
```
...
- 生成/更新发行物清单（联网；不下载 PDF）：`python3 -m scripts.manage.generate_booklet_checklist`
  - 产物：仓库根目录 `BOOKLETS.md`
- 仅按本地文件状态刷新 `BOOKLETS.md` 快捷链接（不联网）：`python3 -m scripts.manage.update_booklets_links_only`
- 为未收集条目生成目录与 `SOURCE.md`（不联网、不下载 PDF）：`python3 -m scripts.manage.collect_missing_booklets`
- 清理无用占位目录（不联网）：
  - dry-run：`python3 -m scripts.manage.prune_unused_booklets`
  - apply：`python3 -m scripts.manage.prune_unused_booklets --apply`
  - 保护：目录内放置空文件 `MANUAL_KEEP` 可避免被清理
- （可选）审计 `booklet_en.md` 的德文占比（不联网）：`python3 -m scripts.translate.audit_booklet_en_language`

### 清单去重与“豪华版本”规则（简述）

`scripts/manage/generate_booklet_checklist.py` 会把官网同一发行物的多个版本页进行去重，并在同组内优先选择“更豪华”的实体版本作为代表条目（价格/蓝光/精装/限量等）。
...
- `python3 -m py_compile scripts/manage/*.py scripts/translate/*.py`
- `python3 -m scripts.manage.update_booklets_links_only`
- `python3 -m scripts.manage.prune_unused_booklets`
- `python3 -m scripts.manage.collect_missing_booklets --limit 1 --dry-run`

在线验证（联网，尽量跑；避免污染仓库文件建议加 `--output`）：

- `python3 -m scripts.manage.generate_booklet_checklist --url '<购买链接1>' --url '<购买链接2>' --output /tmp/BOOKLETS.online-smoke.md`

## 排坑备忘（不常见但有帮助）

- `scripts/translate/extract_booklet_english.py` 的语言过滤是启发式；双语并排/两栏排版时，“残留德文/碎行”属于常见现象，必须回到 `## [PDF p.xx]` 做快速人工清理与核对。
...
- 运行 `python3 -m scripts.manage.update_booklets_links_only` 刷新 `BOOKLETS.md` 的状态与快捷链接
